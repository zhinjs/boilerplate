...
import {App, NSession, OitqEventMap, Plugin} from "oitq";
...
export async function triggerTeach(ctx: Plugin, session: NSession<keyof OitqEventMap,App.MessageEvent>) {
    const teaches = await ctx.app.database.models.QA.findAll()
...
export default function install(plugin: Plugin) {
    plugin.middleware((session: NSession<keyof OitqEventMap,App.MessageEvent>) =>triggerTeach(plugin, session))
}
