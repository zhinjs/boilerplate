...
export default function install(ctx: Plugin) {
    ctx.command('qa [question:string] [answer:string]', 'all')
        .desc('问答管理')
        ...
        .action(async ({session, options,source}, q, a) => {
            ...
            if (options.search) {
                ...
                const data = await ctx.app.database.models.QA.findAll(condition)
            ...
            if (options.detail) {
                const teach = await ctx.app.database.models.QA.findOne({
            ...
            if (options.remove) {
                const dialogue = await ctx.app.database.models.QA.destroy({
                    where: {
                        id: options.id
                    }
                })
            ...
            if (q) {
                ...
                let dialogues = await ctx.app.database.models.QA.findAll({
                    where: {
                        question: q,
                    }
                })
                ...
                dialogue = await ctx.app.database.models.QA.create({
                    ...data,
                    question: q,
                })
